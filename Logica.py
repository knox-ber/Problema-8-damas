...
class Formula :
    ...
    def SATtableaux(self): # IMPLEMENTADO CON BACKTRACKING
        ...
        frontera = [estado]
        # Cambiar este bucle para cambiar de método
        for hijo in estado.expandir():
                resultado = hijo.SATtableaux()
                if resultado != None:
                    return resultado
    
        # Implementación breadth/depth first:
...
class Descriptor :
    ...
    def __init__ (self,args_lista,chrInit=512) -> None:
...
def SATsolver(A):
    # print("Entraré a lit_numero")
    def lit_numero(l):
        if '-' in l:
            return -(ord(l[1:]) - 256)
        else:
            return ord(l) - 256
    # print("Entraré a clausula_numero")
    def clausula_numero(C):
        return [lit_numero(l) for l in C]

    # print("Entraré a fnc_numero")
    def fnc_numero(S):
        return [clausula_numero(C) for C in S]

    # print("Entraré a obtener_int")
    def obtener_int(mod):
        return {chr(256 + abs(n)):n>0 for n in mod}

    # print("Entraré a obtener_numeros")
    ...
    S = tseitin(A)
    #print(f"Post-tseitin: {S}")
    S = fnc_numero(S)
    #print(f"Post-fnc_numero: {S}")
    numeros = obtener_numeros(S)
    #print(f"Post-obtener_numeros: {numeros}")
    solucion = pycosat.solve(S)
    solucion = [x for x in solucion if abs(x) in numeros]
    #print(f"Pycosat: {solucion}")
